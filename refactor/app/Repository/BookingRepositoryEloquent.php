...
class BookingRepositoryEloquent extends BaseRepository implements BookingRepository
{
    ...
    public function getUsersJobsHistory($user_id, Request $request)
    {
        ...
        $pagenum       = "1";//Set default to 1
        $numpages      = 0;//Set default to 0
        $usertype      = null;//Set to null
        $usertype      = '';
        $emergencyJobs = array();
        $noramlJobs    = array();
        $jobs          = array();
    ...
    public function store($user, $data)
    {
        ...
        if ($user->user_type == env('CUSTOMER_ROLE_ID')) {
            ...
            $data['gender']    = $this->getGender($data);
            $data['certified'] = $this->getCertification($data);
    ...
    public function storeJobEmail($data)
    {
        ...
        $this->mailSubject = 'Vi har mottagit er tolkbokning. Bokningsnr: #' . $job->id;
        $this->mailData = [
            'user' => $user,
            'job'  => $job
        ];
        $this->mailer->send($email, $name, $this->mailSubject, 'emails.job-created', $this->mailData);
    ...
    public function jobToData($job)
    {
        ...
        $this->dataBag                           = array();            // save job's information to data for sending Push
        $this->dataBag['job_id']                 = $job->id;
        $this->dataBag['from_language_id']       = $job->from_language_id;
        $this->dataBag['immediate']              = $job->immediate;
        $this->dataBag['duration']               = $job->duration;
        $this->dataBag['status']                 = $job->status;
        $this->dataBag['gender']                 = $job->gender;
        $this->dataBag['certified']              = $job->certified;
        $this->dataBag['due']                    = $job->due;
        $this->dataBag['job_type']               = $job->job_type;
        $this->dataBag['customer_phone_type']    = $job->customer_phone_type;
        $this->dataBag['customer_physical_type'] = $job->customer_physical_type;
        $this->dataBag['customer_town']          = $job->town;
        $this->dataBag['customer_type']          = $job->user->userMeta->customer_type;
        $due_Date                                = explode(" ", $job->due);
        $due_date                                = $due_Date[0];
        $due_time                                = $due_Date[1];
        $this->dataBag['due_date']               = $due_date;
        $this->dataBag['due_time']               = $due_time;
        $this->dataBag['job_for']                = $this->getJobFor($job);
    ...
    public function jobEnd($post_data = array())
    {
        ...
        $this->mailSubject   = 'Information om avslutad tolkning för bokningsnummer # ' . $job->id;
        $this->mailData = [
            'user'         => $user,
            'job'          => $job,
        ...
        $this->mailer->send($email, $name, $this->mailSubject, 'emails.session-ended', $this->mailData);
        ...
        $email                      = $user->email;
        $name                       = $user->name;
        $this->mailData['user']     = $user;
        $this->mailData['for_text'] = 'lön';
    ...
    public function sendSMSNotificationToTranslator($job)
    {
        ...
        Log::info($this->messageForCustomerPhysicalType($job));
    ...
    public function sendPushNotificationToSpecificUsers($users, $job_id, $data, $msg_text, $is_need_delay)
    {
        ...
        ///////////////////////////////////////////////////
        // I would suggest to use the guzzle http client //
        ///////////////////////////////////////////////////
        $ch = curl_init();
    ...
    public function getPotentialTranslators(Job $job)
    {
        ...
        $gender           = $job->gender;
        $translator_level = $this->translatorLevels($job);
    ...
    public function updateJob($id, $data, $cuser)
    {
        ...
        $log_data           = [];
        $langChanged        = false;
        $current_translator = $job->translatorJobRel->whereNull('cancel_at')->orWhereNotNull('completed_at')->first();
    ...
    private function changeTimedoutStatus($job, $data, $changedTranslator)
    {
        ...
        if ($data['status'] == 'pending') {
            ...
            $this->mailSubject = 'Vi har nu återöppnat er bokning av ' . TeHelper::fetchLanguageFromJobId($job->from_language_id) . 'tolk för bokning #' . $job->id;
            $this->mailer->send($email, $name, $this->mailSubject, 'emails.job-change-status-to-customer', $this->mailData);
        ...
        if ($changedTranslator) {
            $job->save();
            $this->mailSubject = 'Bekräftelse - tolk har accepterat er bokning (bokning # ' . $job->id . ')';
            $this->mailer->send($email, $name, $this->mailSubject, 'emails.job-accepted', $this->mailData);
            return true;
        }
    ...
    private function changePendingStatus($job, $data, $changedTranslator)
    {
        ...
        if ($data['status'] == 'assigned' && $changedTranslator) {
            ...
            $this->mailSubject = 'Bekräftelse - tolk har accepterat er bokning (bokning # ' . $job->id . ')';
            $this->mailer->send($email, $name, $this->mailSubject, 'emails.job-accepted', $dataEmail);

            $translator = Job::getJobsAssignedTranslatorDetail($job);
            $this->mailer->send($translator->email, $translator->name, $this->mailSubject, 'emails.job-changed-translator-new-translator', $dataEmail);
        ...
        $this->mailSubject = 'Avbokning av bokningsnr: #' . $job->id;
        $this->mailer->send($email, $name, $this->mailSubject, 'emails.status-changed-from-pending-or-assigned-customer', $dataEmail);
    ...
    private function changeAssignedStatus($job, $data)
    {
        if (in_array($data['status'], ['withdrawbefore24', 'withdrawafter24', 'timedout'])) {
            ...
            if (in_array($data['status'], ['withdrawbefore24', 'withdrawafter24'])) {
                ...
                $this->mailer->send($email, $name, $this->mailSubject, 'emails.job-cancel-translator', $dataEmail);
            }
            $job->save();
            return true;
        }
    ...
    public function sendChangedTranslatorNotification($job, $current_translator, $new_translator)
    {
        ...
        $email             = $user->email;
        $name              = $user->name;
        $this->mailSubject = 'Meddelande om tilldelning av tolkuppdrag för uppdrag # ' . $job->id . ')';
        $this->mailData    = [
            'user' => $user,
            'job'  => $job
        ];
        ...
        $this->mailer->send($email, $name, $this->mailSubject, 'emails.job-changed-translator-customer', $this->mailData);
        if ($current_translator) {
            ...
            $this->mailer->send($email, $name, $this->mailSubject, 'emails.job-changed-translator-old-translator', $this->mailData);
        }
        ...
        $this->mailer->send($email, $name, $this->mailSubject, 'emails.job-changed-translator-new-translator', $this->mailData);
    ...
    public function sendChangedDateNotification($job, $old_time)
    {
        ...
        $this->mailSubject = 'Meddelande om ändring av tolkbokning för uppdrag # ' . $job->id . '';
        $this->mailer->send($email, $name, $this->mailSubject, 'emails.job-changed-date', $this->mailData);

        $translator = Job::getJobsAssignedTranslatorDetail($job);
        $this->mailData['user'] = $translator;
        $this->mailer->send($translator->email, $translator->name, $this->mailSubject, 'emails.job-changed-date', $this->mailData);
    ...
    public function sendChangedLangNotification($job, $old_lang)
    {
        ...
        $this->mailer->send($email, $name, $this->mailSubject, 'emails.job-changed-lang', $this->mailData);
        $translator = Job::getJobsAssignedTranslatorDetail($job);
        $this->mailer->send($translator->email, $translator->name, $this->mailSubject, 'emails.job-changed-date', $this->mailData);
    }
    ...
    public function sendExpiredNotification($job, $user)
    {
        ...
        if ($this->isNeedToSendPush($user->id)) {
            $users_array = array($user);
            $this->sendPushNotificationToSpecificUsers($users_array, $job->id, $data, $this->mailData, $this->isNeedToDelayPush($user->id));
        }
    }
    ...
    public function sendNotificationByAdminCancelJob($job_id)
    {
        ...
        $this->dataBag['customer_phone_type']    = $job->customer_phone_type;
        $this->dataBag['customer_physical_type'] = $job->customer_physical_type;
        $this->dataBag['customer_town']          = $user_meta->city;
        $this->dataBag['customer_type']          = $user_meta->customer_type;
        ...
        $this->dataBag['due_time']               = $due_time;
        $this->dataBag['job_for']                = $this->getJobFor($job);
        $this->sendNotificationTranslator($job, $this->dataBag, '*');   // send Push all sutiable translators
    }
    ...
    {
        $this->dataBag                      = array();
        $this->dataBag['notification_type'] = 'session_start_remind';
        $this->mailData                     = array(
        ...
        if ($this->bookingRepository->isNeedToSendPush($user->id)) {
            $users_array = array($user);
            $this->bookingRepository->sendPushNotificationToSpecificUsers($users_array, $job->id, $this->dataBag, $this->mailData, $this->bookingRepository->isNeedToDelayPush($user->id));
        }
    }
    ...
    public function acceptJobWithId($job_id, $cuser)
    {
        ...
        if (!Job::isTranslatorAlreadyBooked($job_id, $cuser->id, $job->due)) {
            if ($job->status == 'pending' && Job::insertTranslatorJobRel($cuser->id, $job_id)) {
                ...
                $this->dataBag = array();
                $this->dataBag['notification_type'] = 'job_accepted';
                $language = TeHelper::fetchLanguageFromJobId($job->from_language_id);
                $this->mailData = array(
                ...
                if ($this->isNeedToSendPush($user->id)) {
                    $users_array = array($user);
                    $this->sendPushNotificationToSpecificUsers($users_array, $job_id, $this->dataBag, $this->mailData, $this->isNeedToDelayPush($user->id));
                }
    ...
    public function cancelJobAjax($data, $user)
    {
        ...
        */
        ...
        if ($cuser->is('customer')) {
            ...
            if ($translator) {
                $this->dataBag = array();
                $this->dataBag['notification_type'] = 'job_cancelled';
                $language = TeHelper::fetchLanguageFromJobId($job->from_language_id);
                $this->mailData = array(
                ...
                if ($this->isNeedToSendPush($translator->id)) {
                    ...
                    $this->sendPushNotificationToSpecificUsers($users_array, $job_id, $this->dataBag, $this->mailData, $this->isNeedToDelayPush($translator->id));// send Session Cancel Push to Translaotor
                }
            }
            return $response;
        } 
...
//        $jobs = TeHelper::convertJobIdsInObjs($job_ids);
    ...
    public function getAll(Request $request, $limit = null)
    {
        ...
        if ($cuser && $cuser->user_type == env('SUPERADMIN_ROLE_ID')) {
            $allJobs = (new \DTApi\Criteria\SAJobsCriteria($jobsQuery))->getCriteria();
        } else {
            $allJobs = (new \DTApi\Criteria\JobsCriteria($jobsQuery))->getCriteria();
        }
        return $allJobs;
    }
